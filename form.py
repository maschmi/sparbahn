...
reqsession = Session()
reqsession.cookies.set_policy(BlockAll())
...
ht = reqsession.post(base_url,data=payload, proxies=proxies,verify=False)
...
ht = reqsession.get(base_url,params=payload, headers=headers,proxies=proxies, verify=False)
angebote = ht.json()


with open("angebote.json","w") as outfile:
    json.dump(angebote, outfile)
